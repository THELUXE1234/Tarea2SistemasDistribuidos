...
class IoTDevice(Thread):
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            producer.send(topic[0], value=data1)
            producer.send(topic[2], value=data2)
            producer.send(topic[4], value=data3)
            producer.send(topic[1], value=data4)
            producer.send(topic[3], value=data5)
            end_time =  time.time()
